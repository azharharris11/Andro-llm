...
import { User, Zap, Image as ImageIcon, Target, Award, RefreshCw, Sparkles, TrendingUp, DollarSign, MousePointer2, Ghost, Mic, Layers, Cpu, Archive, Search, Lightbulb, PenTool, BookOpen, ArrowRight, MessageCircle, Activity, Video, Magnet, Flame, Heart, CheckCircle2, FileText, Globe } from 'lucide-react';
...
const Node: React.FC<NodeProps> = ({ data, selected, onClick, onAction, isGridView = false }) => {
  ...
  const containerClass = isGridView 
    ...
    : `absolute w-[360px] rounded-xl border-0 node-interactive node-enter flex flex-col ${styles.container} ${selected ? 'z-50 scale-[1.02]' : 'z-10'} ${isValidated ? 'ring-2 ring-emerald-400' : ''}`;
  ...
  return (
    ...
    >
      ...
      <div className={`p-4 flex flex-col gap-3 bg-white/50 rounded-b-xl flex-1 ${isGhost ? 'opacity-50' : ''}`}>
        ...
        {/* MASS DESIRE NODE */}
        {data.type === NodeType.MASS_DESIRE_NODE && data.massDesireData && (
            <div className="mt-1">
                <div className="text-[10px] text-rose-700 bg-rose-50 p-2 rounded border border-rose-100 mb-2">
                    <span className="font-bold block mb-1">Human Need:</span>
                    {data.massDesireData.type}
                </div>
                <div className="text-[10px] text-slate-500 italic">"Market Symptom: {data.massDesireData.marketSymptom}"</div>
            </div>
        )}
        ...
        {/* SALES LETTER / LANDING PAGE DISPLAY */}
        {(data.type === NodeType.SALES_LETTER || data.type === NodeType.LANDING_PAGE_NODE) && data.fullSalesLetter && (
      ...
      <div className="p-1.5 bg-slate-50/80 border-t border-slate-100 rounded-b-xl">
        {!isGridView && !data.prediction && !isGhost && (
            ...
            {data.type === NodeType.ROOT && (
                <div className="flex flex-col gap-1.5">
                    <button 
                        ...
                        className="w-full py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 shadow-sm"
                    >
                        <Flame className="w-3 h-3" /> Start "Desire-First" Discovery
                    </button>
                    {/* ... other root buttons ... */}
            ...
            {/* CREATIVE NODE - ADD LANDING PAGE */}
            {data.type === NodeType.CREATIVE && (
                <div className="flex flex-col gap-1">
                     <button 
                        onClick={(e) => { e.stopPropagation(); onAction('generate_landing_page', data.id); }}
                        className="w-full py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-medium rounded-lg border border-indigo-200 transition-all flex items-center justify-center gap-2 shadow-sm"
                     >
                        <FileText className="w-3 h-3" /> Generate Landing Page
                     </button>
                </div>
            )}
